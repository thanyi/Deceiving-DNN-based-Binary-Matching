...
def get_betweenness_and_random_blocks(binary_path, func_name):
    """获取介数中心性最大的块和随机块地址"""
    ...
    # 计算介数中心性最大的块
    blocks = [(addr, info.get('centrality_betweenness', 0))
    ...
    betweenness_addr = blocks[0][0]

    # 随机选一个基本块（排除介数中心性最大的块，确保不同）
    other_blocks = [addr for addr, score in blocks[1:] if len(blocks) > 1]
    if other_blocks:
        random_addr = random.choice(other_blocks)
    else:
        # 如果只有一个块，就用同一个
        random_addr = betweenness_addr

    return betweenness_addr, random_addr
...
def run_correlation_experiment(junk_bytes=64):
    ...
    print("[*] Comparing: Highest Betweenness Block vs Random Block")
    ...
    # 采样 60 个函数，每个 2 次变异（介数中心性最大块/随机块），共 120 个点
    ...
    temp_dir = "/tmp/proxy_corr_exp_betweenness"
    ...
    for sample in tqdm(samples, desc="Collecting data"):
        ...
        try:
            ...
            # 获取介数中心性最大的块和随机块
            betweenness_addr, random_addr = get_betweenness_and_random_blocks(orig_path, fname)
            if not betweenness_addr or not random_addr:
                continue

            # 对两种块分别插入垃圾指令
            for label, target_addr in [('Betweenness', betweenness_addr), ('Random', random_addr)]:
                patched_path = None
                try:
                    # 插入垃圾指令
    ...
    colors = {'Betweenness': '#e74c3c', 'Random': '#3498db'}
    ...
    legend_elements = [
        Line2D([0], [0], marker='o', color='w', label='Highest Betweenness',
    ...
    plt.title("Proxy Correlation: Highest Betweenness vs Random Block",
              fontsize=15, fontweight='bold', pad=15)
    plt.grid(True, linestyle='--', alpha=0.4)

    save_path = os.path.join(SAVE_DIR, "exp6_betweenness_vs_random.png")
