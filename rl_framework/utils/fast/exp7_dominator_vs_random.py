...
def get_dominator_and_random_blocks(binary_path, func_name):
    """获取支配分数最大的块和随机块地址"""
    ...
    # 从 basic_blocks 中提取支配分数
    # 注意：dominator_scores 不是直接在 data 中，需要从节点属性获取
    blocks = []
    for addr, bb_info in data['basic_blocks'].items():
        dominator_score = bb_info.get('dominator_score', 0)
        blocks.append((addr, dominator_score))
    ...
    dominator_addr = blocks[0][0]

    # 随机选一个基本块（排除支配分数最大的块，确保不同）
    other_blocks = [addr for addr, score in blocks[1:] if len(blocks) > 1]
    if other_blocks:
        random_addr = random.choice(other_blocks)
    else:
        # 如果只有一个块，就用同一个
        random_addr = dominator_addr

    return dominator_addr, random_addr
...
def run_correlation_experiment(junk_bytes=64):
    ...
    print("[*] Comparing: Highest Dominator Score vs Random Block")
    ...
    # 采样 60 个函数，每个 2 次变异（支配分数最大块/随机块），共 120 个点
    ...
    temp_dir = "/tmp/proxy_corr_exp_dominator"
    ...
    for sample in tqdm(samples, desc="Collecting data"):
        ...
        try:
            ...
            # 获取支配分数最大的块和随机块
            dominator_addr, random_addr = get_dominator_and_random_blocks(orig_path, fname)
            if not dominator_addr or not random_addr:
                continue

            # 对两种块分别插入垃圾指令
            for label, target_addr in [('Dominator', dominator_addr), ('Random', random_addr)]:
                patched_path = None
                try:
                    # 插入垃圾指令
    ...
    colors = {'Dominator': '#e74c3c', 'Random': '#3498db'}
    ...
    legend_elements = [
        Line2D([0], [0], marker='o', color='w', label='Highest Dominator',
    ...
    plt.title("Proxy Correlation: Highest Dominator Score vs Random Block",
              fontsize=15, fontweight='bold', pad=15)
    plt.grid(True, linestyle='--', alpha=0.4)

    save_path = os.path.join(SAVE_DIR, "exp7_dominator_vs_random.png")
