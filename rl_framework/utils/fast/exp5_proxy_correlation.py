...
def get_critical_and_edge_blocks(binary_path, func_name):
    """获取关键块和边缘块地址"""
    ...
    scored_blocks = [(addr, info.get('critical_score', 0)) 
    ...
    return scored_blocks[0][0], scored_blocks[-1][0]
...
def run_correlation_experiment(junk_bytes=64):
    ...
    print("[*] Independent of RL mutation module")
    ...
    # 采样 60 个函数，每个 2 次变异 (关键块/边缘块)，共 120 个点
    ...
    for sample in tqdm(samples, desc="Collecting data"):
        ...
        try:
            ...
            # 获取关键块和边缘块
            crit_addr, edge_addr = get_critical_and_edge_blocks(orig_path, fname)
            if not crit_addr or not edge_addr: 
                continue
            
            # 对两种块分别插入垃圾指令
            for label, target_addr in [('Critical', crit_addr), ('Edge', edge_addr)]:
                patched_path = None
                try:
                    # 插入垃圾指令
    ...
    colors = {'Critical': '#e74c3c', 'Edge': '#3498db'}
    ...
    legend_elements = [
        Line2D([0], [0], marker='o', color='w', label='Critical Block', 
               markerfacecolor='#e74c3c', markersize=10),
        Line2D([0], [0], marker='o', color='w', label='Edge Block', 
    ...
    plt.title("Proxy Correlation: Feature Drift vs Attack Effectiveness", 
              fontsize=15, fontweight='bold', pad=15)
    plt.grid(True, linestyle='--', alpha=0.4)
    
    save_path = os.path.join(SAVE_DIR, "exp_proxy_correlation.png")
